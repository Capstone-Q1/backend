...
REQUIRED_AI_DATA_PATHS = [
    ("SETTINGS", "SIMULATION OPTIONS", "Source"),
    ("SETTINGS", "SOURCE POWER CONDITIONS", "PowerH"),
    ("SETTINGS", "PREASURE & INLET CONDITIONS", "Pressure"),
    ("OUTPUT LIST", "TEMPERATURE PARAMETERS", "Electron Temperature"),
]
...
class AiRequestData(BaseModel):
    ...
    def validate_required_paths(self) -> "AiRequestData":
        payload = self.model_dump(by_alias=True)
        missing = [
            ".".join(path)
            for path in REQUIRED_AI_DATA_PATHS
            if _is_blank(_get_nested_value(payload, path))
        ]
        if missing:
            raise ValueError(f"invalid AI request data: missing {missing}")
        return self
