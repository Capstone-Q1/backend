...
from app.features.ask_question.exceptions import DatabaseOperationException
from app.models.query_response_log import QueryResponseLog
from app.models.solver_result import SolverResult
from app.models.chat_session import ChatSession


def _commit_and_refresh(db: Session, row, *, error_message: str):
    try:
        db.commit()
        db.refresh(row)
        return row
        ...
        db.rollback()
        raise DatabaseOperationException(error_message) from exc


def _commit(db: Session, *, error_message: str) -> None:
    try:
        db.commit()
    except SQLAlchemyError as exc:
        db.rollback()
...
def create_query_log(
    ...
    *,
    ...
    return _commit_and_refresh(
        db,
        row,
        error_message="failed to save query log",
    )
...
def find_solver_results_by_log_file_names(
    ...
    *,
    ...
    try:
        ...
        db.rollback()
        raise DatabaseOperationException("failed to query solver results") from exc
...
def update_query_response(
    ...
    *,
    ...
    try:
        ...
        db.rollback()
        raise DatabaseOperationException("failed to update query response") from exc
    if row is None:
        raise DatabaseOperationException(
            f"query_response_log not found: log_id={log_id}"
        )
    ...
    return _commit_and_refresh(
        db,
        row,
        error_message="failed to update query response",
    )
...
def delete_chat_session_by_id(db: Session, *, session_id: int, user_id: str) -> bool:
    ...
    # query_response_log가 chat_sessions를 참조하므로 자식 로그를 먼저 삭제한다.
    db.query(QueryResponseLog).filter(
        QueryResponseLog.session_id == session_id,
        QueryResponseLog.user_id == user_id,
    ).delete(synchronize_session=False)

    db.delete(chat_session)
    _commit(db, error_message="failed to delete chat session")
    return True
...
def update_chat_session_title(
    ...
    *,
    ...
    return _commit_and_refresh(
        db,
        chat_session,
        error_message="failed to update chat session title",
    )
...
def update_chat_session_updated_at(db: Session, *, session_id: int) -> None:
    ...
    _commit(db, error_message="failed to update chat session timestamp")
